...
- (UILabel *)xAxisLabelWithText:(NSString *)text atIndex:(NSInteger)index {
    ...
    labelXAxis.contentMode = UIViewContentModeTop;
    ...
    if (self.positionYAxisRight) {
        ...
        center = CGPointMake(positionOnXAxis, self.frame.size.height - lRect.size.height/2 - 5);//扩展方法，间隔5像素
    }
...
- (void)drawYAxis {
    ...
    if (self.autoScaleYAxis) {
        ...
        for (NSNumber *dotValue in dotValues) {
            ...
            NSLog(@"formattedValue: %@",formattedValue);
...
//暂时每个点的位置
- (CGPoint)showLabelAtPoint:(NSInteger)index{
    CGFloat  dotValue = [self.dataSource lineGraph:self valueForPointAtIndex:index];
    CGFloat  positionOnXAxis = (((self.frame.size.width - self.YAxisLabelXOffset) / (numberOfPoints - 1)) * index) + self.YAxisLabelXOffset;
    CGFloat  positionOnYAxis = [self yPositionForDotValue:dotValue];
    CGPoint  tmpPoint = CGPointMake(positionOnXAxis, positionOnYAxis);
    return tmpPoint;
}
